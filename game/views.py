    ...
    if request.method == 'POST':
        ...
        if username and password:
            if Player.objects.filter(username=username).exists():
                return render(request, 'game/register.html', {'error': 'Имя пользователя занято'})
...
def play_view(request):
    ...
    if request.user.is_authenticated:
        role = 'single'
        if mode == 'multiplayer':
            role = request.session.get('role', 'host')
...
def api_save_game(request):
    if request.method == 'POST':
        try:
            ...
            if request.user.is_authenticated:
                request.user.playerXp = data.get('xp', request.user.playerXp)
                request.user.currentRankIdx = data.get('rankIdx', request.user.currentRankIdx)
                request.user.lightbulbs = data.get('bulbs', request.user.lightbulbs)
                request.user.playerGold = data.get('gold', request.user.playerGold)
...
def api_mp_join(request, room_id):
    ...
    if request.method == 'POST':
        try:
            room = MultiplayerRoom.objects.get(id=room_id, status='waiting')
...
def api_mp_sync(request, room_id):
    ...
    try:
        ...
        if request.method == 'GET':
            if room.status == 'waiting' and is_host:
                room.save()
